...
def generateMCM(window1,window2,binLo,binHi, trimAtL=10000,mbbFilename = None,transfer = None,  binningWeightMap = None, type=0,std=0,pixWin=True,arrayTag=None,spTag=None,ellStart=None):
    """
    window: data window
    ...
    if transfer != None:
        ...
        if len(transfer)==3:
            ell,f_ell_Pol,transfer_pw=transfer
        else:
        ...
        transferTrim_Cross=transferTrim_Pol.copy()
        transferTrim_TT=transferTrim_Pol.copy()
        if len(transfer)==3:
            transferTrim_TT.powerMap[:]*=transfer_pw.powerMap[:]
            transferTrim_Cross.powerMap[:]*=numpy.sqrt(transfer_pw.powerMap[:])
...
#print "Total time in trimshift calls %f secs"%deltaT
    ...
    larray = numpy.arange(ellStart,numpy.int(trimAtL)+ellStart)
    ...
    delta = numpy.min([deltaLx,deltaLy])/3.0
